...
const Reply = React.memo(
  ({ reply, canInteract, reviewId, setReplies, depth = 0 }) => {
    ...
    const handleSubmitReply = async () => {
      ...
      try {
        ...
        setReplies((prevReplies) => {
          return prevReplies.map((r) => {
            if (
              r._id === (parentReplyId || _id) ||
              r.replies.some(
                (subReply) => subReply._id === (parentReplyId || _id)
              )
            ) {
    ...
    return (
      <div
        className={cn("reply relative mt-2", parentReplyId ? "pl-8" : "pl-2")}
      >
        <div
          className={cn("rounded-medium bg-content1 p-4 shadow-small w-full")}
        >
        ...
        {isReplying && (
          <div
            className={cn(
              "mt-2 rounded-medium bg-content1 p-3 shadow-small relative pl-8"
            )}
          >
            <Textarea
              fullWidth
